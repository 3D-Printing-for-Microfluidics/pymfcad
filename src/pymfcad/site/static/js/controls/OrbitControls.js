...
class OrbitControls extends Controls {
  ...
  constructor(object, domElement = null) {
    ...
    this._keyModifiers = { shift: false, ctrl: false, meta: false };
  ...
  stopListenToKeyEvents() {
    if (this._domElementKeyEvents !== null) {
      ...
      this._domElementKeyEvents.removeEventListener("keyup", this._onKeyUp);
      this._domElementKeyEvents = null;
    }
  }
  ...
  _handleKeyState() {
    ...
    const modifier =
      this._keyModifiers.shift ||
      this._keyModifiers.ctrl ||
      this._keyModifiers.meta;
    if (modifier) {
      if (!this.enablePan) return;
      this._pan(-dx * this.keyPanSpeed, dy * this.keyPanSpeed);
      return;
    }

    if (!this.enableRotate) return;
    const height = this.domElement?.clientHeight || 1;
    const angle = (_twoPI * this.keyRotateSpeed) / height;
    if (dx !== 0) this._rotateLeft(-dx * angle);
    if (dy !== 0) this._rotateUp(dy * angle);
  }
  ...
  _handleKeyDown(event) {
    this._keyModifiers.shift = event.shiftKey;
    this._keyModifiers.ctrl = event.ctrlKey;
    this._keyModifiers.meta = event.metaKey;

    if (event.code === this.keys.UP) this._keysPressed.up = true;
    if (event.code === this.keys.BOTTOM) this._keysPressed.down = true;
    if (event.code === this.keys.LEFT) this._keysPressed.left = true;
    if (event.code === this.keys.RIGHT) this._keysPressed.right = true;
    ...
    if (event.code === "MetaLeft" || event.code === "MetaRight")
      this._keyModifiers.meta = true;

    if (
      event.code === this.keys.UP ||
      event.code === this.keys.BOTTOM ||
      event.code === this.keys.LEFT ||
      event.code === this.keys.RIGHT
    ) {
  ...
  _handleKeyUp(event) {
    if (event.code === this.keys.UP) this._keysPressed.up = false;
    if (event.code === this.keys.BOTTOM) this._keysPressed.down = false;
    if (event.code === this.keys.LEFT) this._keysPressed.left = false;
    if (event.code === this.keys.RIGHT) this._keysPressed.right = false;

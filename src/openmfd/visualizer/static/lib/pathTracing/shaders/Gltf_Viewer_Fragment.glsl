...
uniform int uCustomLightCount;
uniform vec4 uCustomLightPos[4]; // xyz: position, w: type (0=dir,1=point,2=spot)
uniform vec4 uCustomLightDir[4]; // xyz: direction, w: cos(angle) for spot
uniform vec4 uCustomLightColor[4]; // rgb: color, w: intensity
uniform vec2 uCustomLightParams[4]; // x: distance, y: decay
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	// Ground plane disabled for floating model
...
//-----------------------------------------------------------------------------------------------------------------------------
{
    	...
    	for (int bounces = 0; bounces <50; bounces++)
	{
		...
		if (hitType == DIFF) // Ideal DIFFUSE reflection
		{
			...
			// direct lighting from custom lights + ambient
			vec3 directCol = vec3(0.0);
			if (uAmbientIntensity > 0.0)
			{
				directCol += uAmbientColor * uAmbientIntensity;
			}
			for (int i = 0; i < 4; i++)
			{
				if (i >= uCustomLightCount) break;
				vec3 lightPos = uCustomLightPos[i].xyz;
				float lightType = uCustomLightPos[i].w;
				vec3 lightDir = normalize(uCustomLightDir[i].xyz);
				float cosAngle = uCustomLightDir[i].w;
				vec3 lightColor = uCustomLightColor[i].rgb;
				float lightIntensity = uCustomLightColor[i].w;
				float maxDist = 1e20;
				float atten = 1.0;
				vec3 L = vec3(0.0);
				...
				else
				{
					...
					L = toLight / maxDist;
					float distLimit = uCustomLightParams[i].x;
					float decay = uCustomLightParams[i].y;
					if (distLimit > 0.0)
					{
						float d = max(maxDist, 0.0001);
						atten = pow(clamp(1.0 - (d / distLimit), 0.0, 1.0), decay > 0.0 ? decay : 1.0);
					}
				}
				...
				if (lightType > 1.5)
				{
					...
					if (spotCos < cosAngle) continue;
				}

				float ndotl = max(0.0, dot(L, nl));
				if (ndotl <= 0.0) continue;

				rayOrigin = x + nl * epsIntersect;
				rayDirection = L;
				float shadowT = SceneIntersect();
				if (shadowT == INFINITY || shadowT > maxDist - epsIntersect)
				{
					directCol += lightColor * lightIntensity * ndotl * atten;
				}
			}

			accumCol += mask * directCol;
			break;

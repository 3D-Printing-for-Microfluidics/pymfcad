...
function setRenderSize(width, height, overridePixelRatio)
{
	SCREEN_WIDTH = width;
	SCREEN_HEIGHT = height;
	if (Number.isFinite(overridePixelRatio))
		pixelRatio = overridePixelRatio;
...
function requestPathTracingRender(options)
{
	...
	const exposure = Number.isFinite(options?.exposure) ? options.exposure : null;
	const ratio = Number.isFinite(options?.pixelRatio) ? options.pixelRatio : pixelRatio;

	return new Promise((resolve) => {
		const start = performance.now();
		const waitForReady = () =>
		{
			if (renderer && pathTracingRenderTarget && pathTracingUniforms?.uResolution)
			{
				exportRestore = {
					width: SCREEN_WIDTH,
				...
				setRenderSize(width, height, ratio);
				cameraIsMoving = true;
				sampleCounter = 1.0;
				frameCounter = 1.0;
...
function initTHREEjs()
{
	...
	screenOutputUniforms = {
		...
		uBackgroundColor: { type: "v3", value: new THREE.Color(0x222222) }
	};
	if (typeof backgroundColor !== 'undefined' && backgroundColor && screenOutputUniforms.uBackgroundColor) {
		screenOutputUniforms.uBackgroundColor.value.copy(backgroundColor);
	}
...
function animate()
{
	...
	if (exportRequest && sampleCounter >= exportRequest.targetSamples)
	{
		...
		renderer.domElement.toBlob((blob) =>
		{
			if (exportRestore)
			{
				if (exportRestore.exposure !== undefined)
					renderer.toneMappingExposure = exportRestore.exposure;
				setRenderSize(exportRestore.width, exportRestore.height, exportRestore.pixelRatio);
				exportRestore = null;
			}
			resolve(blob);
		}, 'image/png');

...
let defaultModelPaths = [
	"models/device__v1.glb",
	// "models/void_control__v1.glb", "models/void_fluidic__v1.glb"
];
let modelPaths = defaultModelPaths.slice();
let pendingModelPaths = null;
let hdrReady = false;
let modelLoadStarted = false;
const embeddedInParent = window.parent && window.parent !== window;
let pendingCameraState = null;
let pendingLightState = null;
const MAX_CUSTOM_LIGHTS = 4;
const customLightState = {
	count: 0,
	positions: Array.from({ length: MAX_CUSTOM_LIGHTS }, () => new THREE.Vector4()),
	directions: Array.from({ length: MAX_CUSTOM_LIGHTS }, () => new THREE.Vector4(0, 1, 0, -1)),
	colors: Array.from({ length: MAX_CUSTOM_LIGHTS }, () => new THREE.Vector4()),
	params: Array.from({ length: MAX_CUSTOM_LIGHTS }, () => new THREE.Vector2()),
	ambientColor: new THREE.Color(0, 0, 0),
	ambientIntensity: 0,
};

function sanitizeModelPaths(paths) {
...
function startModelLoad(paths) {
	if (modelLoadStarted) return;
	const cleaned = sanitizeModelPaths(paths);
...
function setModelPathsFromParent(paths) {
	pendingModelPaths = sanitizeModelPaths(paths);
	if (!pendingModelPaths.length) return;
	if (hdrReady) {
		startModelLoad(pendingModelPaths);
	}
}

function applyModelTransform(vec) {
	const v = vec.clone();
	v.multiplyScalar(modelScale);
	v.applyAxisAngle(new THREE.Vector3(0, 1, 0), modelRotationY);
	v.applyAxisAngle(new THREE.Vector3(1, 0, 0), modelRotationX);
	v.add(modelPositionOffset);
	return v;
}

function applyCameraState(state) {
	if (!state || !state.pos || !state.target) return;
	if (!worldCamera || !cameraControlsObject || !cameraControlsYawObject || !cameraControlsPitchObject) {
	...
	const worldPos = applyModelTransform(pos);
	const worldTarget = applyModelTransform(target);

	cameraControlsObject.position.copy(worldPos);

	const dir = worldTarget.clone().sub(worldPos).normalize();
	const yaw = Math.atan2(-dir.x, -dir.z);
	const pitch = -Math.asin(Math.max(-1, Math.min(1, dir.y)));
	cameraControlsYawObject.rotation.y = yaw;
	cameraControlsPitchObject.rotation.x = pitch;
	const rollDeg = Number.isFinite(state.roll) ? state.roll : 0;
	const rollRad = THREE.MathUtils.degToRad(rollDeg) + Math.PI;
	cameraControlsYawObject.rotation.z = rollRad;

	if (Number.isFinite(state.fov)) {
		worldCamera.fov = Math.max(1, Math.min(179, state.fov));
		worldCamera.updateProjectionMatrix();
		fovScale = worldCamera.fov * 0.5 * (Math.PI / 180.0);
		if (pathTracingUniforms?.uVLen && pathTracingUniforms?.uULen) {
			pathTracingUniforms.uULen.value = pathTracingUniforms.uVLen.value * worldCamera.aspect;
		}
	}

	const focus = worldTarget.distanceTo(worldPos);
	if (Number.isFinite(focus) && focus > 0) {
		focusDistance = focus;
		if (pathTracingUniforms?.uFocusDistance) {
...
function applyLightState(state) {
	...
	const dirLights = Array.isArray(state.directional) ? state.directional : [];
	const ambient = state.ambient || { color: [0, 0, 0], intensity: 0 };

	const ambientColor = Array.isArray(ambient.color)
		? new THREE.Color().fromArray(ambient.color)
		: new THREE.Color(ambient.color || 0x000000);
	customLightState.ambientColor.copy(ambientColor);
	customLightState.ambientIntensity = Number.isFinite(ambient.intensity) ? ambient.intensity : 0;

	customLightState.count = Math.min(MAX_CUSTOM_LIGHTS, dirLights.length);
	for (let i = 0; i < MAX_CUSTOM_LIGHTS; i++) {
		const entry = dirLights[i];
		if (!entry) {
			customLightState.positions[i].set(0, 0, 0, 0);
			customLightState.directions[i].set(0, 1, 0, -1);
			customLightState.colors[i].set(0, 0, 0, 0);
			customLightState.params[i].set(0, 1);
			continue;
		}
		const pos = new THREE.Vector3(entry.position.x, entry.position.y, entry.position.z);
		const target = new THREE.Vector3(entry.target.x, entry.target.y, entry.target.z);
		const worldPos = applyModelTransform(pos);
		const worldTarget = applyModelTransform(target);
		const dir = worldTarget.clone().sub(worldPos).normalize();

		const typeValue = entry.type === 'spot' ? 2 : entry.type === 'point' ? 1 : 0;
		customLightState.positions[i].set(worldPos.x, worldPos.y, worldPos.z, typeValue);
		const cosAngle = Number.isFinite(entry.angle) ? Math.cos(entry.angle) : -1;
		customLightState.directions[i].set(dir.x, dir.y, dir.z, cosAngle);
		customLightState.colors[i].set(entry.color[0], entry.color[1], entry.color[2], entry.intensity);
		customLightState.params[i].set(
			Number.isFinite(entry.distance) ? entry.distance : 0,
			Number.isFinite(entry.decay) ? entry.decay : 1
		);
	}

	if (pathTracingUniforms?.uCustomLightCount) {
		pathTracingUniforms.uCustomLightCount.value = customLightState.count;
		pathTracingUniforms.uCustomLightPos.value = customLightState.positions;
		pathTracingUniforms.uCustomLightDir.value = customLightState.directions;
		pathTracingUniforms.uCustomLightColor.value = customLightState.colors;
		pathTracingUniforms.uCustomLightParams.value = customLightState.params;
		pathTracingUniforms.uAmbientColor.value.copy(customLightState.ambientColor);
		pathTracingUniforms.uAmbientIntensity.value = customLightState.ambientIntensity;
	}
...
window.addEventListener('message', (event) => {
	...
	if (payload.type === 'openmfd-pathtracing-models') {
		setModelPathsFromParent(payload.modelPaths || []);
		return;
	}
	if (payload.type === 'openmfd-pathtracing-camera') {
		applyCameraState(payload.camera);
		return;
	}
	if (payload.type === 'openmfd-pathtracing-lights') {
		if (pathTracingUniforms?.uCustomLightCount) {
			applyLightState(payload.lights);
		} else {
			pendingLightState = payload.lights;
		}
	}
});
...
let modelRotationX = -Math.PI / 2; // align with Z-up world
// let modelPositionOffset = new THREE.Vector3(50,20,50);
...
function prepareGeometryForPT(meshList, pathTracingMaterialList, triangleMaterialMarkers)
{
	...
	modelMesh.geometry.rotateX(modelRotationX);
...
function initSceneData() 
{
	...
	pathTracingUniforms.uCustomLightCount = { value: 0 };
	pathTracingUniforms.uCustomLightPos = { value: customLightState.positions };
	pathTracingUniforms.uCustomLightDir = { value: customLightState.directions };
	pathTracingUniforms.uCustomLightColor = { value: customLightState.colors };
	pathTracingUniforms.uCustomLightParams = { value: customLightState.params };
	pathTracingUniforms.uAmbientColor = { value: customLightState.ambientColor };
	pathTracingUniforms.uAmbientIntensity = { value: customLightState.ambientIntensity };
	...
	if (pendingLightState) {
		applyLightState(pendingLightState);
...
function updateVariablesAndUniforms()
{
	...
	worldCamera.getWorldPosition(camPos);
	worldCamera.getWorldDirection(camDir);
	const focusPoint = camPos.clone().add(camDir.clone().multiplyScalar(focusDistance));
	const fmt = (v) => Number.isFinite(v) ? v.toFixed(2) : '0.00';
	const posText = `Pos: (${fmt(camPos.x)}, ${fmt(camPos.y)}, ${fmt(camPos.z)})`;
	const dirText = `Dir: (${fmt(camDir.x)}, ${fmt(camDir.y)}, ${fmt(camDir.z)})`;
	const tgtText = `Focus: (${fmt(focusPoint.x)}, ${fmt(focusPoint.y)}, ${fmt(focusPoint.z)})`;
	cameraInfoElement.innerHTML =
		"FOV: " + worldCamera.fov + " / Aperture: " + apertureSize.toFixed(2) + " / FocusDistance: " + focusDistance +
		"<br>" + posText + "<br>" + dirText + "<br>" + tgtText + "<br>Samples: " + sampleCounter;
...
hdrTexture = hdrLoader.load(
	...
	function (texture)
	{
		...
		hdrReady = true;
		if (pendingModelPaths && pendingModelPaths.length) {
			startModelLoad(pendingModelPaths);
		} else if (!embeddedInParent) {
			startModelLoad(modelPaths);
		}
	}
);

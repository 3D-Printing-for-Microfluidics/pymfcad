...
class Router:
    ...
    def autoroute_channel(self, input_port:Port, output_port:Port, label:str):
    ...
    def route_with_fractional_path(self, input_port:Port, output_port:Port, route:list[tuple[float, float, float]], label:str):
        if input_port.parent is None:
            raise ValueError("Port must be added to component before routing! (input)")
        if output_port.parent is None:
            raise ValueError("Port must be added to component before routing! (output)")
        ...
        # get locations
        start_loc = input_port.position
        end_loc = output_port.position
        ...
        for r in route:
            ...
            sum_y += y
            sum_z += z
            path.append((start_loc[0] + round(sum_x*diff[0]), start_loc[1] + round(sum_y*diff[1]), start_loc[2] + round(sum_z*diff[2])))

        if sum_x != 1.0 or sum_y != 1.0 or sum_z != 1.0:
            raise ValueError(f"Fractional routing components must sum to (1.0, 1.0, 1.0) currently ({sum_x}, {sum_y}, {sum_z})")

        # path to constant cross-section polychannel shapes
        polychannel_shapes = self._path_to_polychannel_shapes(input_port, output_port, path)
    ...
    def route(self):
        print("Routing...")
        new_routes = []
        for name, route_info in self.routes.items(): # Route cached paths if valid

    ...
    for c in range(x):
        for r in range(y):
            ...
            if r != 0:
                ...
                continue
            try:
                device.add_shape(autoroute_channel(device, v1.get_port("F_OUT"), v2.get_port("F_IN"), channel_size=chan_size, channel_margin=chan_size, nettype="fluidic"))
            except TypeError:
                pass
            try:
                device.add_shape(autoroute_channel(device, v1.get_port("P_OUT"), v2.get_port("P_IN"), channel_size=chan_size, channel_margin=chan_size, nettype="pneumatic"))
            except TypeError:
                pass
